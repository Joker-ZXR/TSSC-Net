...
import models.transformer_configs as configs
from models.path_generate import generate2d, gilbert2d, generate3d, gilbert3d, gilbert3d_dir, generate_slicewise_hilbert_indices, \
    generate_gilbert_indices_3D
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3, input_dim=3, old=1):
        ...
        self.config = config
        img_size = _pair(
            img_size)  # alter this to no longer use _pair; in my case it is 3D, where dimensions don't match
        ...
        self.positional_encoding = nn.Parameter(torch.zeros(1, n_patches*2, config.hidden_size))
        self.dropout = Dropout(config.transformer["dropout_rate"])
    ...
    def forward(self, x):
        ...
        x = depthEmbedding(x)
        x = x.flatten(
            2)  # Check if this dimension needs to be altered; currently changes shape from (B, C, H, W) to (B, C, H*W)
        x = x.transpose(-1, -2)
...
class ART_block(nn.Module):
    ...
    def forward(self, x):
        if self.transformer:
            ...
            transformer_out = transformer_out.contiguous().view(B, hidden, h, w, 2)
            # upsample output
...
class ResViT(nn.Module):
    def __init__(self, config, input_dim, img_size=224, output_dim=3, vis=False):
        ...
        mult = 2 ** (n_downsampling - i)
        model = []
        model = [nn.Conv3d(ngf * mult*2, int(ngf * mult / 2),
        ...
        model = []
        model = [nn.ReplicationPad3d(3)]
        model += [nn.Conv3d(int(ngf/2), output_dim, kernel_size=7, padding=0)]
        # model += [nn.Tanh()]
        model += [nn.Sigmoid()]
...
class MambaLayer(nn.Module):
    ...
    def __init__(self, dim, d_state=16, d_conv=4, expand=2):  # Before it was d_state=16, d_conv=4, expand=2
        ...
        # direction_1--width,  scan_x
        self.generator_x = gilbert3d_dir(64, 64, 8, stage='x')  # Before it was 64, 64, 8 (better one is 32, 32, 32)
        self.gilbert_indices_x = generate_gilbert_indices_3D(64, 64, 8, self.generator_x).expand(-1, dim, -1).permute(0, 2,1)  # Before it was 64, 64, 8
        self.degilbert_indices_x = torch.argsort(self.gilbert_indices_x)
        self.gilbert_r_indices_x = torch.flip(self.gilbert_indices_x, dims=[2])
        self.degilbert_r_indices_x = torch.argsort(self.gilbert_r_indices_x)
        self.mamba_x = BiMambaBlock(dim, d_state, d_conv, expand)

        # direction_2--height,  scan_y
        self.generator_y = gilbert3d_dir(64, 64, 8, stage='y')  # Before it was 64, 64, 8 (better one is 32, 32, 32)
        self.gilbert_indices_y = generate_gilbert_indices_3D(64, 64, 8, self.generator_y).expand(-1, dim, -1).permute(0, 2,1)  # Before it was 64, 64, 8
        self.degilbert_indices_y = torch.argsort(self.gilbert_indices_y)
        self.gilbert_r_indices_y = torch.flip(self.gilbert_indices_y, dims=[2])
        self.degilbert_r_indices_y = torch.argsort(self.gilbert_r_indices_y)
        self.mamba_y = BiMambaBlock(dim, d_state, d_conv, expand)

        # direction_3--depth,  scan_z
        self.generator_z = gilbert3d_dir(64, 64, 8, stage='z')  # Before it was 64, 64, 8 (better one is 32, 32, 32)
        self.gilbert_indices_z = generate_gilbert_indices_3D(64, 64, 8, self.generator_z).expand(-1, dim, -1).permute(0, 2,1)  # Before it was 64, 64, 8
        self.degilbert_indices_z = torch.argsort(self.gilbert_indices_z)
        self.gilbert_r_indices_z = torch.flip(self.gilbert_indices_z, dims=[2])
        self.degilbert_r_indices_z = torch.argsort(self.gilbert_r_indices_z)
        self.mamba_z = BiMambaBlock(dim, d_state, d_conv, expand)

        # fusion
        self.fusion = nn.Sequential(
            nn.Conv3d(dim * 6, dim * 2, kernel_size=3, padding=1),
            nn.GELU(),
    ...
    def forward(self, x):
        ...
        # # Tridirectional mamba
        x1 = x.view(B, C, -1).permute(0, 2, 1)

        # direction_1--x
        self.gilbert_indices_x = self.gilbert_indices_x.to(device)
        x1 = torch.gather(x1, 1, self.gilbert_indices_x)
        x_fwd, x_bwd = self.mamba_x(x1)
        self.degilbert_indices_x = self.degilbert_indices_x.to(device)
        self.degilbert_r_indices_x = self.degilbert_r_indices_x.to(device)
        outx_fwd = torch.gather(x_fwd, 1, self.degilbert_indices_x).permute(0, 2, 1).view(B, C, H, W, D)
        outx_bwd = torch.gather(x_bwd, 1, self.degilbert_r_indices_x).permute(0, 2, 1).view(B, C, H, W, D)
        # out_x = (outx_fwd+outx_bwd)/2

        # direction_2--y
        self.gilbert_indices_y = self.gilbert_indices_y.to(device)
        y1 = torch.gather(x1, 1, self.gilbert_indices_y)
        y_fwd, y_bwd = self.mamba_y(y1)
        self.degilbert_indices_y = self.degilbert_indices_y.to(device)
        self.degilbert_r_indices_y = self.degilbert_r_indices_y.to(device)
        outy_fwd = torch.gather(y_fwd, 1, self.degilbert_indices_y).permute(0, 2, 1).view(B, C, H, W, D)
        outy_bwd = torch.gather(y_bwd, 1, self.degilbert_r_indices_y).permute(0, 2, 1).view(B, C, H, W, D)
        # out_y = (outy_fwd+outy_bwd)/2

        # direction_3--z
        self.gilbert_indices_z = self.gilbert_indices_z.to(device)
        z1 = torch.gather(x1, 1, self.gilbert_indices_z)
        z_fwd, z_bwd = self.mamba_z(z1)
        self.degilbert_indices_z = self.degilbert_indices_z.to(device)
        self.degilbert_r_indices_z = self.degilbert_r_indices_z.to(device)
        outz_fwd = torch.gather(z_fwd, 1, self.degilbert_indices_z).permute(0, 2, 1).view(B, C, H, W, D)
        outz_bwd = torch.gather(z_bwd, 1, self.degilbert_r_indices_z).permute(0, 2, 1).view(B, C, H, W, D)
        # out_z = (outz_fwd+outz_bwd)/2

        # concatenated = torch.cat((out_x, out_y, out_z), dim=1)
        concatenated = torch.cat((outx_fwd, outx_bwd, outy_fwd, outy_bwd, outz_fwd, outz_bwd), dim=1)
...
class BiMambaBlock(nn.Module):
    ...
    def forward(self, x):
        fwd = self.forward_mamba(self.norm(x))
        bwd_x = torch.flip(x, dims=[1])
        bwd = self.backward_mamba(self.norm(bwd_x))
        return fwd, bwd
...
class cmMambaWithCNN(nn.Module):
    ...
    def __init__(self, config, in_channels, d_state=16, d_conv=4, expand=2, ngf=64, norm_layer=nn.BatchNorm2d,
                 use_bias=True):
        super().__init__()
        # Mamba block
...
class TriMamba(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.encoder_1(x)
        x2 = self.encoder_2(x1)
        x3 = self.encoder_3(x2) # [1, 256, 256, 64,8]
...
CONFIGS = {
    'Mamba-B_16': configs.get_b16_config(),
    'Mamba-L_16': configs.get_l16_config(),
    'Res-Mamba-B_16': configs.get_resmamba_b16_config(),  # This one is used by the authors
    'Res-Mamba-L_16': configs.get_resmamba_l16_config(),
}
